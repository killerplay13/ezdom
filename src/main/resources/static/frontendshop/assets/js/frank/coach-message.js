...
// ====================== 留言板資訊 ====================== //
const req = 'http://localhost:8080/ezdom/coach/message/' + url_coachId;
...
function showCoachMessage() {
    ...
    for(let data of coachMessage){
        ...
        div.innerHTML = `
            ...
            <textarea id="updateText" style="margin-left:65px; resize:none; width:300px; height:60px;" type="text" class="task_name_update -none" placeholder="請輸入內容…">${data.content}</textarea>
...
function add() {
    ...
    const req = "http://localhost:8080/ezdom/coach/message/add";
...
function update(){
    const req = "http://localhost:8080/ezdom/coach/message/update/" + updateMessageId;
    fetch(req, {
        method: 'put',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
...
function deleteMessage(){
    const req = "http://localhost:8080/ezdom/coach/message/delete/" + deleteMessageId;
    fetch(req, {
        method: 'delete',
        headers: { 'Content-Type': 'application/json' }
        // body: JSON.stringify({
